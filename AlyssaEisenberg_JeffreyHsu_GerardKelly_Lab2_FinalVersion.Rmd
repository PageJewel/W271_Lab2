...
\item[dayweek:] is the day of the week for the observation. 10 participants begin the study on Monday, 7 on Tuesday, 19 on Wednesday, 15 on Thursday, 16 on Friday, 6 on Saturday, and 16 on Sunday
...
\item[desired:] measures the participants' desire to drink, with a higher score meaning a greater desire. It has 22 distinct values ranging from 1 to 8 in $\frac{1}{3}$ steps. Note that there are 3 missing values for participant ids 2, 110, 116.

\item[state:] is our measure for state self-esteem, which is a short-term view of self-worth. This was measured daily, unlike \textbf{rosn} long-term self-esteem. It is a continuous variable taking 25 distinct values ranging from 2.3 to 5. Note that there are 3 missing values for participant ids 2, 4, 110.
...
ggplot(dehart, aes(numall, fill = bins, colour = bins)) + geom_density(alpha=0.2) + 
  ggtitle("Self esteem, positive romantic relationship, number of drinks")
ggplot(dehart, aes(desired, fill = bins, colour = bins)) + geom_density(alpha=0.2) +
  ggtitle("Self esteem, positive romantic relationship, number of drinks")
```
...
```{r}
# Create final model
pois_final <- glm(formula = numall ~ nrel_f + rosn + nrel_f:rosn + dayweek_f, data=dehart, family=poisson(link=log))
